...
public class PromptConfig {
    ...
    public PromptTemplate groupPromptTemplate() {
        return new PromptTemplate("""
        너는 사용자의 검색 결과를 분석하여, 의미적으로 일관되고 검색 추천에 적합한 "그룹 이름"을 생성하는 엔진이다.
        ...
        [목표]
        - 사용자가 검색창에 입력하기 좋은 짧고 명확한 그룹명을 1개 생성하라.
        - 생성된 그룹명은 keyword와 0.8 이상의 의미 유사도를 가지며,
          results의 공통 주제나 분위기를 압축적으로 표현해야 한다.

        [작성 규칙]
        1. keyword를 그대로 반복하지 말고, 의미를 확장하거나 함축한 형태로 변형하라.
        2. "추천", "모음", "작", "리스트" 같은 상투적 단어는 사용하지 말라.
        3. 문장형 표현(예: "읽고 싶은 책")은 금지하고,
           검색어 형태(예: "로맨스 소설", "감성 에세이")로만 작성하라.
        4. 3~6 단어 이내의 짧은 명사구 형태로 작성하라.
        5. 반드시 한글로 작성하라.
        6. 출력은 오직 하나의 JSON 객체만 포함해야 한다.
        7. 코드블록(````json`, ``` 등)이나 백틱(`)을 절대 포함하지 말라.
        8. 출력은 다음 JSON 형식을 따라야 한다:
        {{
          "groupName": "여기에 생성된 그룹명"
        }}
        """);
    }
