...
public class SearchService {
    ...
    public Page<SearchResponse> search(String keyword, Pageable pageable) throws JsonProcessingException {
        ...
        List<Book> reranked = aiApiClient.rerank(keyword, vectorBooks);
        String keywordJson = objectMapper.writeValueAsString(keywordBooks.stream().map(Book::toJson).toList());
        String vectorJson = objectMapper.writeValueAsString(reranked.stream().map(Book::toJson).toList());
        ...
        publisher.publishEvent(new CreateGroupEvent(books,ids, embeddingResponse.getEmbedding()));
